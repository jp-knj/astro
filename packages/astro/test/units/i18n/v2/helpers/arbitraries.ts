...
export const pathArbitrary = (): fc.Arbitrary<string> =>
	fc
		.array(pathSegmentArbitrary(), { minLength: 0, maxLength: 5 })
		.map((segments) => '/' + segments.join('/'));

export const queryParamsArbitrary = (): fc.Arbitrary<string> =>
	fc.option(
		fc
			.dictionary(fc.stringMatching(/^[a-zA-Z0-9_]+$/), fc.stringMatching(/^[a-zA-Z0-9-_]+$/), {
				minKeys: 0,
				maxKeys: 3,
			})
			.map((params) => {
...
export const acceptLanguageArbitrary = (): fc.Arbitrary<string | undefined> =>
	fc.option(
		fc
			...
			.map((locales) =>
				locales
					.map(([locale, q]) => (q !== undefined ? `${locale};q=${q.toFixed(1)}` : locale))
					.join(', '),
			),
		{ freq: 50 },
	);
...
export const fallbackArbitrary = (
	locales: Locale[],
): fc.Arbitrary<Partial<Record<Locale, Locale[]>> | undefined> =>
	fc.option(
		fc
			.dictionary(
...
export const domainsArbitrary = (
	locales: Locale[],
): fc.Arbitrary<Record<Locale, string> | undefined> =>
	fc.option(
		fc.dictionary(
			fc.constantFrom(...locales),
			fc
				.tuple(fc.constantFrom('http', 'https'), fc.stringMatching(/^[a-z0-9-]+\.[a-z]{2,}$/))
		...
		.array(
			fc.tuple(
				...
				fc.record({
					...
					locales: fc.option(fc.shuffledSubarray(locales, { minLength: 1 }), { freq: 70 }),
				}),
			),
